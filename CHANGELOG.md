...
- Settings: `describe` publishes every environment variable in `protocol.environment`. `LASSO_FEED_TOKEN` (a redacted secret) and `LASSO_HTTP_TIMEOUT` join `LASSO_FORMAT`.
- Demo: `task import <url>` imports titles from a JSON feed over HTTP, previewed and confirmed like any mutation. HTTP failures map to the catalog: 401/403 `auth_failure`, 404 `not_found`, 408/429 and timeouts `transient_failure`, network and 5xx `service_unavailable`.
