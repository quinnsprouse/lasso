...
In JSON mode, expected errors also go to **stdout** (one parseable stream); the exit code still reports failure. A consumer that closes stdout early (`| head`) never turns into an error: the run keeps the exit code of the outcome it was writing (0 for a success, 4 for a confirmation) and writes nothing to stderr. Once the terminal outcome is written, nothing follows it, not even an interrupt that arrives mid-write. A run that can never finish (every fiber waiting on something nothing will complete) ends as `internal_error`, exit 70, instead of exiting silently. `confirmArgs` is the canonical continuation — replay it verbatim; `confirmCommand` is display-only.
...
3. Stale token (state changed since the preview) → `stale_confirmation`, exit 64. Re-plan. A value that is not a token at all is `invalid_usage`. A replay that fails for an environmental reason (unreadable store, outage) keeps that failure's own code, exit, and `fix`.
