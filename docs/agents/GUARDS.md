...
Commands are split into approximate argv segments: quotes group text; newlines, `;`, `|`, `&`, and `(` split segments; a `$(…)` or backtick substitution stays one unit of its word (its body has its own quoting, so a `)` or `"` inside it cannot break the outer command) and its body is judged as a command of its own, recursively, as are `sh -c` and `eval` bodies; a single-quoted or backslash-escaped `$` or backtick is literal text and never opens a body; a `#` comment is dropped; a quoted heredoc body (`<<'EOF'`) is literal and skipped while an unquoted one has its substitutions judged; leading shell keywords (`if`, `while`, `do`, `function name`, …) are stripped; a backslash-newline continuation is deleted as the shell does; more than four levels of nesting is refused rather than left unjudged; `env`, `sudo`, `command`, `exec`, `time`, `nice`, `nohup`, `builtin`, `xargs`, `corepack` wrappers and `VAR=value` prefixes are stripped; each segment is judged by argv. The rules are a table in `guard.mjs` and `test/unit/guard.test.ts` runs every row below.
